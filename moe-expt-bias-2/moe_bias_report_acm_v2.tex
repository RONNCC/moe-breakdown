...
\paragraph{Verdict.}
The causal check is \emph{mixed}. For two of three MoE models (OLMoE,
Phi-3.5-MoE) the $\phi$-ranked attribution does predict where bias-relevant
shift is removable---faster than random ablation, supporting the
interpretation of the concentration metrics; for Mixtral the high-routing
control dominates, so expert selection by traffic beats attribution there.
No model exhibits a tiny, stable, ``fixable'' expert set consistent with
surgical pruning: even the best $\phi$ curve (OLMoE) needs ~10\% of experts
ablated before half the gap is removed, with large perplexity cost
(selectivity collapses from $0.24$ at $k{=}102$ to $0.03$ at $k{=}512$ on
OLMoE). This narrows the paper's claim one step further: $\phi$ ranks
bias-relevant experts on two of three MoE models, but ``prune the bias
experts'' remains unsupported because no small ablation removes a large
share of the bias without destroying capability. Full curves are tabulated
in Appendix~B.
...
router. The causal ablation (Section~\ref{sec:causal}) partially
...
Mixtral, where traffic-based selection dominates, and no curve supports
pruning a small expert set. The demographic follow-up reinforces the
...
``Reviewer-Prioritized Extensions'') defines three experiments that speak to
...
\paragraph{Experiment 4/6 --- causal expert ablation (DONE for 4 models;
Mixtral-GPT-OSS-Gemma-DBRX ladder extension PENDING).}
...
\textbf{Status: DONE for four models.} Ablation curves exist on disk at
\path{results/{exp1-concentration-olmoe-1b-7b,exp1-concentration-mixtral-8x7b,
exp1-concentration-phi3.5-moe,exp2-dense-baseline-olmo-7b}/experiment6/
...
OLMo-7B dense ($n{=}60$, $N{=}32$ layer players) has no high-routing control
(no router) and overshoots at $k{=}1$ (drop $1.083$), reflecting its
32-player partition. The full-curve analysis is reported in
Section~\ref{sec:causal}. Ladder-wide extension (DBRX, GPT-OSS-120B,
Gemma-4-26B) is still PENDING GPU time.
...
\paragraph{Experiment 8 --- same-mechanism comparison (DONE at the summary
level; per-pair analysis PENDING).}
...
(OLMoE, $n_{pairs}=100$, $N=16$ FFN-layer players, $H = 0.7513329831783248$,
Gini $= 0.614543153196013$) and
\path{results/exp8-lloo-phi3.5-moe/result.json} (Phi-3.5-MoE,
$n_{pairs}=50$, $N=32$, $H = 0.8829717705954556$, Gini $=
0.4792328068088605$). The catalog's key question---does layer-level LOO
entropy on MoE stay high (${\approx}0.90$) or drop toward the dense band
$0.63$--$0.76$?---has a partial answer: OLMoE's $H_{\mathrm{lloo}} = 0.751$
lands inside the dense band, while Phi-3.5-MoE's $H_{\mathrm{lloo}} = 0.883$
stays near the MoE range. \textbf{Status: DONE for summary concentration
metrics (both result.json files present); PENDING for any per-pair analysis
--- the exp8 directories contain only \path{player\_ids.json},
\path{result.json}, and slurm logs, with no per-pair $\phi$ payloads, so
bootstrap CIs and permutation tests are not yet derivable for these runs.}

\paragraph{Net status for reviewers.}
Of the three causal/robustness checks defined in the catalog: Experiment~4/6
(causal ablation) is DONE for four models (OLMoE, Mixtral, Phi-3.5-MoE, and
the OLMo-7B dense control; Section~\ref{sec:causal}) with the remaining
ladder rungs PENDING; Experiment~7 (proxy agreement) is DONE and reports a
null result on all three tested models (per-pair $\rho \approx 0$ on
Mixtral, OLMoE, and Phi-3.5-MoE); and Experiment~8
(same-mechanism) is DONE at the summary-metric level with per-pair analysis
PENDING. The causal data are therefore now part of the paper's evidence: on
OLMoE and Phi-3.5-MoE the $\phi$-ranked ablation drops the bias gap faster
than random ablation, on Mixtral the high-routing-frequency control
outperforms $\phi$, and no model supports a small fixable expert set.
