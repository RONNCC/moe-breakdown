...
contrast, the smallest dense-to-MoE entropy gap is $0.031$ (vs.\ the
null-bias Gemma rung, the lowest MoE point; excluding that rung it is
$0.119$); the dense/MoE split is the robust contrast of the study.
...
(\texttt{s04\_bootstrap\_cis.json}). Eleven models have per-pair: six MoE v1 captures, the Exp~5 OLMoE capture, and four dense v0 captures (no per-pair payloads for dense v0; dense v1 reruns are in progress).
...
(\texttt{concentration\_metrics}). The dense baselines are the v0 captures
used in Table~\ref{tab:dense}, read from the \path{exp2-*-v0} result.jsons
(\texttt{concentration\_metrics}); v1 dense reruns exist on disk
(1800/4000/1800/1800 pairs) but the paper reports the v0 figures. Bootstrap
CIs are 
not derivable for dense rows: no per-pair $\phi$ payloads for dense runs
exist on disk, so \path{s04_bootstrap_cis.json} covers the MoE ladder only
(bootstrapping a block statistic requires resampling the pairs that produced
the per-pair $\phi$ values, which is exactly the payload that dense runs never
shipped); those cells are therefore marked `---' and flagged
\texttt{CI:pending} in Section~\ref{sec:limits}. GPT-OSS-120B shows its
valid 2000-pair v1 capture. The k/N values are the verified
...
\midrule
OLMo-7B (dense)$^{*}$ & olmo & 32 & --- & 1.0 & exp2-dense-baseline-olmo-7b & 0.7194078268195452 & 0.6929742140401861 & --- & --- & 0.6867990542672688 & 0.6051147043350128 & --- & --- \\
Phi-3.5-Mini (dense)$^{*}$ & phi3.5 & 32 & --- & 1.0 & exp2-dense-baseline-phi3.5-mini & 0.7579053226812761 & 0.6130531225167948 & --- & --- & 0.6247542059965518 & 0.5277236074309628 & --- & --- \\
Llama-2-7B (dense)$^{*}$ & llama & 32 & --- & 1.0 & exp2-dense-crosscheck-llama-2-7b & 0.650629997969714 & 0.6995207212132746 & --- & --- & 0.7317225612544715 & 0.6920951785852945 & --- & --- \\
Llama-3.1-8B (dense)$^{*}$ & llama & 32 & --- & 1.0 & exp2-dense-crosscheck-llama-3.1-8b & 0.6300970845941072 & 0.7299968143336719 & --- & --- & 0.7486734941342921 & 0.6983218590825292 & --- & --- \\
\midrule
...
(\texttt{abs\_dH}/\texttt{abs\_dG}). Dense rows: v0 captures from the
\texttt{exp2-*-v0} result.jsons as reported in Table~\ref{tab:dense}; CIs
are not derivable because no per-pair $\phi$ payloads exist for dense runs
(\texttt{s04\_bootstrap\_cis.json} covers the MoE ladder only), flagged
\texttt{CI:pending} in Section~\ref{sec:limits}. Exp~5 row: pooled OLMoE
concentration from
...
\texttt{routing\_contrast} ranking and the exact-Shapley ranking on Mixtral,
where exact computation is tractable ($K{=}2/8$ experts active, $2^8 = 256$
coalitions), over $n{=}20$ pairs and the first and last MoE layer.
\textbf{Status: DONE.} The result exists on disk at
\path{results/exp1-concentration-mixtral-8x7b/experiment7/
experiment7_proxy_agreement.json}: \texttt{mean\_spearman\_rho} of
$-0.085$ (layer 0, sd $0.373$) and $+0.058$ (layer 31, sd $0.445$) over
$n{=}20$ evaluated pairs. The per-pair agreement is statistically
indistinguishable from zero at both layers --- the proxy ranking does not
reproduce exact-Shapley ordering within this model/pair budget,
consistent with the noisy small-$k$ regime of the causal curves in
...
null result (per-pair $\rho \approx 0$ on Mixtral); and Experiment~8
