...
**Full-pipeline reproducibility gate (verified 2026-08-11).** Re-running the
entire documented pipeline (s01, s02, s03, s04, s05, s06, s07, s08,
`paper_figures_seaborn.py`, `audit_appendix.py`) fresh on the committed
tree reproduced **all 8 `outputs/*.json` byte-identically** and **5/6
figures byte-identically**; `audit_appendix.py` passed 12/12. The single
drift, `figures/s02_js_distribution.png`, was a stale committed artifact
(generated by an earlier s02 edit — regenerating from HEAD script + data
yields a different but **byte-stable** PNG, confirmed identical across two
consecutive runs) and has been refreshed in git. The dated machine-readable
record of this gate is `stats_analysis/outputs/reproducibility_check.json`
(per-artifact md5s, environment versions, drift explanation). Re-run the
gate before submission: execute the §3 commands in order, then `md5` the
outputs/figures and diff against that record.
