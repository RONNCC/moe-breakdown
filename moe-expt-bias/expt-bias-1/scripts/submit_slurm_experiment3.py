#!/usr/bin/env python3
"""Submit an Experiment 3 (Shapley interaction/synergy check) job to Slurm.
...
run_experiment_script.sbatch with run_experiment3_interactions.py as SCRIPT.

Usage:
  python3 scripts/submit_slurm_experiment3.py --config configs/study.olmoe.concentration.yaml
  python3 scripts/submit_slurm_experiment3.py --config configs/study.mixtral-8x7b.concentration.yaml --max-pairs 40
  python3 scripts/submit_slurm_experiment3.py --config configs/study.olmoe.concentration.yaml --dry-run
"""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Submit Experiment 3 (Shapley interaction/synergy check) to Slurm")
    ...
    p.add_argument("--max-pairs", type=int, default=20,
                   help="Prompt subsample size (2^K forwards/pair/layer) (default: 20)")
    p.add_argument("--max-layers", type=int, default=2,
                   help="Number of MoE layers to test, evenly spaced (default: 2)")
    p.add_argument("--result-dir", default=None,
                   help="Override result dir (default: output_root/study_name from config)")
    p.add_argument("--out-dir", default=None,
                   help="Override output dir directly (default: result_dir/experiment3)")
    ...
    out_dir = Path(args.out_dir).expanduser() if args.out_dir else (result_dir / "experiment3")
    ...
    extra_args = f"--out-dir {out_dir} --max-pairs {args.max_pairs} --max-layers {args.max_layers}"

    export_bits = {
        "ALL": None,
        "STUDY_CONFIG": str(config_path.resolve()),
        "SCRIPT": "scripts/run_experiment3_interactions.py",
    ...
    job_name = f"bias-exp3-{cfg.study_name}"
