#!/usr/bin/env python3
"""Submit an Experiment 6 (ladder-wide ablation: phi/freq/random controls) job to Slurm.
...
run_experiment_script.sbatch with run_experiment6_ablation.py as SCRIPT.

Usage:
  python3 scripts/submit_slurm_experiment6.py --config configs/study.dbrx.concentration.yaml
  python3 scripts/submit_slurm_experiment6.py --config configs/study.gemma4-26b.concentration.yaml --dry-run
"""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Submit Experiment 6 (ladder-wide ablation) to Slurm")
    ...
    p.add_argument("--routing-freq-pairs", type=int, default=200,
                   help="Pairs for routing-freq computation if absent (default: 200)")
    ...
    extra_args = (
        f"--result-dir {result_dir} --max-pairs {args.max_pairs} "
        f"--routing-freq-pairs {args.routing_freq_pairs}"
    )
