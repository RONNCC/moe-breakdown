...
\subsection{Robustness and Reviewer-Response Analyses (in progress)}
\label{sec:robustness}
To address the methodological limitations identified in our primary experiments — in particular, the granularity confound in Exp2, the limited causal breadth in Exp4, and the unvalidated proxy assumption in routing-contrast — we are running three targeted extensions, whose preliminary structure is described below.

\textbf{Experiment 6 (Ladder-Wide Causal Ablation).} We extend Exp4's physical zero-ablation validation from OLMoE alone to the full ladder: Mixtral-8x7B, Phi-3.5-MoE, and OLMo-7B dense ($n=60$ pairs each; $n=100$ for OLMoE). Critically, each run includes two control conditions automatically generated from Exp1/2 outputs: (i) a \textbf{random-order} ablation that removes experts in a random permutation, and (ii) a \textbf{high-routing-frequency} ablation that removes experts ranked by mean routing weight averaged across all prompt pairs \textit{without} bias-gap scaling (i.e., the most frequently activated experts regardless of bias signal). The flat-early, steep-late curve found in Exp4 for OLMoE is only a causal signature of diffuse bias if it is \textit{slower} than the random control and not simply a function of removing the most-used compute.

\textbf{Experiment 7 (Proxy-vs-Causal Agreement).} The routing-contrast Shapley formulation used in Exp1 is a fast, correlational proxy: it requires only two forward passes per prompt pair (vs.\ $2^K$ for exact active-coalition Shapley). To validate this proxy, we compute the \textbf{Spearman rank correlation $\rho$} between $|\hat{\phi}_{\text{routing-contrast}}|$ and $|\hat{\phi}_{\text{exact}}|$ on the active expert set of Mixtral-8x7B ($K=2/8$, so $2^8 = 256$ coalitions per token — tractable). We evaluate on 20 prompt pairs at the first and last MoE layers. If $\rho \geq 0.7$, the proxy is trustworthy and the concentration claims from Exp1 hold without requiring exact computation across the entire ladder.

\textbf{Experiment 8 (Same-Mechanism Comparison).} Exp2's H2 comparison confounds the player granularity (experts vs.\ layers) with the attribution method (routing-contrast vs.\ LOO). To disentangle these, we re-run OLMoE-1B-7B and Phi-3.5-MoE using \textit{layer-level LOO} (the same method applied to dense baselines in Exp2), forcing $N=32$ players for all models regardless of MoE/dense status. If $H_{\text{LOO}}(\text{OLMoE}) \approx H_{\text{routing-contrast}}(\text{OLMoE}) \approx 0.90$, the Exp2 method confound does not explain the H2 gap and the localizability result stands. If $H_{\text{LOO}}(\text{OLMoE})$ drops toward the dense H ($0.63$–$0.76$), it suggests routing-contrast inflates H by distributing attribution across a much larger player set (1024 experts vs.\ 32 layers).
...
Ongoing work (see Section~\ref{sec:robustness}) addresses the primary open methodological questions: Exp6 tests whether the Exp4 causal ablation curve replicates across the full ladder with random/frequency controls; Exp7 validates the routing-contrast proxy against exact Shapley rankings; and Exp8 re-runs the MoE-vs-dense baseline using a single consistent layer-level LOO attribution method to isolate the granularity confound from a genuine H gap.
