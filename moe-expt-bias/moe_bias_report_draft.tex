...
\begin{itemize}
    ...
    \item \textbf{No Expert-Level Localization Signal (Selectivity Collapse):} Exact Spearman rank-correlation between routing-contrast proxy and causal rankings yields $\rho \approx 0$ ($\rho \in [-0.085, +0.058]$ across layers, Exp~7). Physical zero-ablation of the top $9.9\%$ of proxy-ranked experts yields model-dependent results: OLMoE $+57\%$ bias reduction at a $233\%$ perplexity cost (selectivity $= 0.24$); Phi-3.5-MoE $+30\%$ at $+22\%$ perplexity (selectivity $= 1.35$); Mixtral $-12\%$ (proxy \emph{increases} bias). Frequency-ranked ablation matches or exceeds the proxy in $2/3$ models with no theoretical targeting. The ablation curve is the model-damage curve: bias and general capability are coupled with no separable localization signal (Exp~6);
...
\end{itemize}
Our causal ablation and proxy-validation experiments demonstrate that social bias is a systemic property deeply coupled with core language capabilities in sparse MoEs. Bias has no separable expert-level substrate: the routing-contrast proxy does not map causal contributions ($\rho \approx 0$, Exp~7); proxy ablation selectivity is low or negative ($0.24$ for OLMoE, negative for Mixtral, $1.35$ for Phi-3.5-MoE); and frequency-ranked ablation provides comparable or superior debiasing in $2/3$ models with no targeting. Single-expert debiasing is therefore not a viable alignment strategy.
\end{abstract}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Verdict:} \textbf{No Expert-Level Localization; Surgically Inseparable.} The routing-contrast proxy does not predict causal expert rankings ($\rho \in [-0.085, +0.058]$, Exp~7). At $k \approx 10\%$ ablation, proxy selectivity is low or negative (OLMoE: $0.24$, Mixtral: $-0.18$, Phi-3.5-MoE: $1.35$); frequency-ranked ablation matches or exceeds the proxy in $2/3$ models (Exp~6). Bias is \textbf{not surgically separable} from general capacity.
    \end{itemize}
...
The concentration metric successfully discriminates: we observe relatively concentrated profiles ($H \approx 0.66 - 0.76$) across the dense baseline controls, compared to the highly diffuse ($H \approx 0.88 - 0.92$) profiles observed across MoEs. This significant and stable gap empirically confirms that near-maximum MoE diffuseness is a genuine, robust architectural property and not a floor compression artifact of our measurement setup.
...
To causally validate our game-theoretic findings and evaluate if social stereotypes can be surgically separated from language capabilities, we perform physical expert zero-ablation on the top-$k$ experts ranked by the routing-contrast proxy across the full sparsity ladder (Experiment~6). We run three conditions per model: proxy-top-$k$ (phi-ranked), frequency-top-$k$ (experts ranked by mean routing frequency), and random-$k$. To quantify the efficiency of debiasing, we introduce a \textbf{selectivity metric} defined as:
...
The resulting three-condition curves (proxy-top-$k$, frequency-top-$k$, random-$k$) establish the following per-model picture at the $k \approx 10\%$ ablation level:
\begin{itemize}
    \item \textbf{OLMoE-1B-7B} ($k=102/1024$, $n=60$ pairs): proxy $\text{disparity drop} = +0.567$ ($57\%$ bias reduction), perplexity increase $+233\%$, selectivity $= 0.24$; frequency control: $+0.374$; random: $-0.064$. Large bias drop comes at the price of catastrophic capability loss.
    \item \textbf{Phi-3.5-MoE} ($k=51/512$, $n=30$ pairs): proxy $\text{drop} = +0.302$ ($30\%$), perplexity $+22\%$, selectivity $= 1.35$ (best in the ladder); frequency: $-0.219$; random: $-0.364$. Phi-ranked ablation is the only condition that debias at this level.
    \item \textbf{Mixtral-8x7B} ($k=26/256$, $n=30$ pairs): proxy $\text{drop} = -0.121$ (phi-ranked \emph{increases} bias), perplexity $+66\%$; frequency control: $+0.663$ ($66\%$ bias reduction); random: $-0.005$. The routing-contrast proxy \textbf{inverts} for Mixtral: high-frequency ablation dramatically outperforms phi-ranked ablation. No reliable proxy localization signal.
\end{itemize}

These curves establish that \textbf{there are no dedicated ``bias experts'' in sparse MoEs}. Experiment~7 confirms that the proxy ranking has near-zero correlation with causal expert contributions ($\rho \approx 0$). Critically, frequency-ranked ablation outperforms or matches phi-ranked ablation in two of three models (Mixtral: dramatically; OLMoE: frequency achieves 37\% reduction at $k=10\%$ without the proxy's theoretical motivation), confirming that the proxy provides no reliable targeting advantage. Targeted expert ablation therefore cannot succeed: the proxy does not identify distinct bias hubs, and any debiasing at $k \geq 10\%$ reflects removal of the general high-traffic backbone rather than a localized bias substrate.
...
\begin{enumerate}
    ...
    \item \textbf{No Expert-Level Localization Signal—Surgically Inseparable:} There are no dedicated bias experts. Social bias is \textbf{not surgically separable} because it rides on the same high-traffic, general-capability hubs required for core language function. Selectivity is low across the board: the best case (Phi-3.5-MoE: 1.35) means every $1\%$ of bias removed costs $0.74\%$ of perplexity; for OLMoE selectivity is $0.24$ ($1\%$ bias removal costs $4.2\%$ perplexity); for Mixtral the proxy selectivity is negative. Critically, frequency-ranked ablation provides comparable or superior bias reduction in $2/3$ MoE models without targeting any theoretically-motivated localization signal.
\end{enumerate}
Together with Experiment~7 ($\rho \approx 0$), these results converge on a single conclusion: social bias is a deeply integrated, capability-coupled property of sparse MoEs. There is no expert-level localization signal; the ablation curve is the model-damage curve, not a bias map.
...
\textbf{Experiment 6 (Ladder-Wide Causal Ablation).} We executed physical zero-ablation sweeps across the full ladder using three conditions per model: proxy-top-$k$ (ranked by routing-contrast Shapley value), frequency-top-$k$ (ranked by raw mean routing frequency), and random-$k$ (random expert order). At the $k \approx 10\%$ ablation level, proxy-top-$k$ yields a disparity drop of $+57\%$ for OLMoE (selectivity $= 0.24$; perplexity $+233\%$), $+30\%$ for Phi-3.5-MoE (selectivity $= 1.35$; perplexity $+22\%$), and $-12\%$ for Mixtral (proxy \emph{increases} bias; selectivity $= -0.18$). Frequency-top-$k$ achieves $+66\%$ disparity drop for Mixtral and $+37\%$ for OLMoE at the same ablation budget—matching or exceeding the proxy in $2/3$ models—confirming that the proxy identifies high-traffic backbone experts, not a distinct bias substrate. Selectivity is low or negative in all three MoE models, confirming that bias and general capability are inseparable at expert granularity. (Dense OLMo-7B: perplexity increases $+3879\%$ at $k=4/32$ players, confirming dense models have no comparable ablation corridor.)
...
The most significant contribution of this study is the empirical refutation of the modular-debiasing myth. While sparse MoEs offer elegant computational partitioning, our findings show that this modularity \textbf{does not translate into localized fairness properties}. The routing-contrast proxy has near-zero Spearman correlation with causal expert rankings ($\rho \approx 0$, Exp~7); proxy ablation at $k=9.9\%$ yields low or negative selectivity across all three MoE models tested ($0.24$, $-0.18$, and $1.35$ for OLMoE, Mixtral, and Phi-3.5-MoE respectively), and frequency-ranked ablation achieves comparable or superior debiasing in 2/3 models (Exp~6); early-layer synergy exceeds $70\%$ (Exp~3). \textbf{Surgical expert editing is therefore an ineffective alignment strategy.} There is no separable expert subset that carries bias independently of general capability; the ablation that removes bias is the same as the ablation that degrades the model.
...
\begin{itemize}
    ...
    \item \textbf{Correlational Proxy vs. Causal Validation:} Our routing-contrast formulation remains a correlational proxy. Causal validation via physical zero-ablation (Experiment~6) spans the full sparsity ladder ($n = 30$--$60$ pairs per model), but pair-count variance is non-negligible: OLMoE's disparity drop at $k=10\%$ varied from $18\%$ ($n=100$) to $57\%$ ($n=60$) across sampling draws, indicating the estimates are sensitive to which prompt pairs are included. The qualitative ordering across models and the direction of the capability-coupling result are robust; exact magnitude estimates should be interpreted with this sampling caveat.
...
We present a post-hoc game-theoretic bias-attribution study on sparse Mixture-of-Experts architectures. Across a diverse model ladder spanning massive scales and routing sparsities, we show that social bias is an interactive, demographic-specific, and capability-coupled property of MoEs. The routing-contrast proxy has near-zero Spearman correlation with causal expert rankings ($\rho \approx 0$, Exp~7); proxy ablation selectivity is low or negative across all three MoE models (OLMoE: $0.24$, Mixtral: $-0.18$, Phi-3.5-MoE: $1.35$) and frequency-ranked ablation provides comparable or superior debiasing in $2/3$ models (Exp~6). There is no expert-level bias-localization signal: the ablation curve is the model-damage curve, not a bias map.

Our validation and robustness experiments (see Section~\ref{sec:robustness}) address the primary methodological caveats: Exp~6 delivers ladder-wide causal ablation with three conditions (proxy, frequency, random) and perplexity tracking; Exp~7 measures direct proxy-vs-causal rank agreement ($\rho \approx 0$, establishing the proxy as a routing-frequency marker rather than a causal map); and Exp~8 re-evaluates the MoE-vs-dense gap under a consistent layer-level causal LOO mechanism, where diffuseness attenuates but persists without formal significance testing.
