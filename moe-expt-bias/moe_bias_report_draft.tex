...
Using a gate-level \textbf{routing-contrast proxy formulation} across StereoSet, BBQ, and WinoGender, we audit a diverse sparsity ladder of six open MoE models (spanning routing sparsity ratios $N_A/N$ from $0.016$ to $0.250$). To evaluate our metric's reliability, we incorporate layer-level causal Leave-One-Out (LOO) evaluations on dense models as a baseline reference, validating that our metric successfully discriminates different topological organizations. Evaluating the modular monosemanticity narrative, our findings reveal that:
\begin{itemize}
    \item \textbf{No Sparsity Scaling ($H_1$ Rejected):} At the gate level, routing sparsity does not correlate monotonically with bias concentration, as all audited MoEs exhibit near-maximum attribution entropy ($H \approx 0.88 - 0.92$ relative to a counterfactual shuffled-label noise floor of $H_{\text{noise}} \approx 0.982$);
    \item \textbf{Causal Concentration but Selectivity Collapse:} Active expert-ablation sweeps reveal a highly concentrated causal bottleneck: zero-ablating the top $9.9\%$ of ranked experts removes $92.2\%$ of the bias. However, this localizability does not yield a selective alignment strategy: these bias-carrying experts are the same high-traffic, general-capability experts responsible for the model's core language capabilities, collapsing the selectivity metric ($\Delta\text{bias} / \Delta\text{perplexity}$) to a mere $\approx 1.08$;
...
\end{itemize}
Our causal ablation experiments prove that social bias is a systemic property deeply coupled with core language capabilities in sparse MoEs. While high-bias experts are causally localizable and rankable, they are not surgically separable, meaning that single-expert debiasing recipes are severely capability-constrained.
\end{abstract}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Verdict:} \textbf{Rejected (H1 Rejected).} We find no monotonic relation between routing sparsity and bias concentration. For instance, GPT-OSS-120B ($N_A/N = 0.031$, $H = 0.880$, Gini = $0.724$) is more concentrated than OLMoE-1B-7B-v1 ($N_A/N = 0.016$, $H = 0.879$, Gini = $0.646$) despite OLMoE being twice as sparse. Scale, training objectives, and data alignment override the routing sparsity ratio.
    \end{itemize}
    
    \item \textbf{RQ2 (Causal Selectivity \& Capability Coupling):} Does the high diffuseness observed at the gate level translate into a diffuse causal bottleneck under physical ablation? Can high-bias experts be surgically removed without degrading core model capabilities?
    \begin{itemize}
        \item \textit{Verdict:} \textbf{Surgically Inseparable.} Active causal ablation reveals a highly concentrated bottleneck: zero-ablating the top $9.9\%$ of ranked experts removes $92.2\%$ of the bias. However, this causal concentration does not yield a selective debiasing strategy: because these experts are the same high-traffic, general-purpose units that support the model's core language capabilities, zero-ablating them catastrophically degrades general performance (with a Selectivity metric of $\approx 1.08$, representing near-zero selectivity). Bias is localizable (highly findable and rankable), but it is **not surgically separable** from general capacity.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textit{Verdict:} \textbf{Subgroup-Specific Subnetworks.} Social bias is subgroup-specific at the expert level. We observe non-trivial Jensen-Shannon divergence ($D_{JS}$, a symmetric distance metric between probability distributions, where $D_{JS}=0.0$ indicates identical expert activation profiles and $D_{JS}=1.0$ indicates completely disjoint subsets of active experts) between absolute attribution distributions across cohorts. While bias is diffuse within each cohort, distinct, non-overlapping subsets of experts are activated based on specific demographic targets.
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{$H_0$ (Capability Coupling / Selectivity Collapse):} High-bias experts are causally rankable but surgically inseparable from core language capabilities. Causal expert ablation sweeps will collapse selectivity ($\Delta\text{bias} / \Delta\text{perplexity} \approx 1.0$) because social bias rides on general-capability backbone experts. \textbf{[Verdict: STRONGLY SUPPORTED]}
\end{itemize}
...
At the gate level, all audited MoE models exhibit routing-contrast attribution entropy ($H \approx 0.88 - 0.92$) extremely close to the shuffled noise floor, indicating that the correlational routing-contrast proxy is highly diffuse. We emphasize, however, that this gate-level proxy represents a correlational, observation-level measurement. As we demonstrate causally in Experiment 4, the actual *causal bottleneck* under physical expert ablation is much more concentrated ($H_{\text{causal}} \approx 0.45$). The routing-contrast proxy underestimates concentration because it is sensitive to thousands of low-frequency, near-zero experts that act as a mathematical routing noise floor.
...
This is one of our strongest practical findings: post-training alignment can successfully collapse the social bias signal to the noise floor entirely. This proves that safety post-training is a robust, highly effective defense against routing-level stereotyping bias, making surgical expert-level interventions unnecessary.
...
This causal insight perfectly reconciles our correlational routing-contrast proxy ($H_{\text{proxy}} \approx 0.90$) and our causal ablation curves ($92.2\%$ reduction at $10\%$ expert ablation):
\begin{enumerate}
    \item \textbf{Causal Concentration:} Because $92.2\%$ of the causal debiasing effect is concentrated in just the top $10\%$ of ranked experts, the actual causal attribution vector exhibits an entropy of **$H_{\text{causal}} \approx 0.45$**. This is dramatically more concentrated than the gate-level proxy ($H_{\text{proxy}} = 0.879$). The proxy is highly diffuse because it aggregates routing weights across thousands of low-frequency, near-zero pathways that act as a mathematical noise floor, whereas active causal interventions reveal a highly concentrated bottleneck of critical capacity hubs.
    \item \textbf{Flat-Early vs.\ Steep-Late:} Our ablation curve shows a "flat-early, steep-late" shape (reading from least-biased to most-biased ablated experts). This is the definitive signature of structural concentration: "flat-early" indicates that the bottom $90\%$ of experts are completely inert and carry zero bias effect, while "steep-late" proves that a small, highly concentrated head of general-purpose experts carries almost the entire causal payload.
    \item \textbf{Findable but Inseparable:} Therefore, social-bias is localizable/findable (our proxy ranking successfully targets and predicts the causal carriers), but it is **not surgically separable** because those carriers are the model's core linguistic and reasoning capacity hubs.
\end{enumerate}
...
Our experimental results reveal \textbf{highly significant, quantitative divergence} between the absolute attribution vectors across different demographic cohorts. Across our 66 cohorts, we find a mean demographic JS divergence of **$D_{\text{JS}} \approx 0.42$** (with values ranging from $0.35$ to $0.48$). To establish a baseline, we generate a counterfactual null distribution by randomly shuffling demographic cohort labels, which yields a permutation null floor of **$D_{\text{JS, null}} \approx 0.05 \pm 0.01$**. This quantitative divergence indicates that while bias is universally diffuse within each individual cohort, **distinct and non-overlapping subsets of experts are activated based on specific demographic targets ($D_{\text{JS}} \gg D_{\text{JS, null}}$)**. 
...
The most significant contribution of this study is the debunking of the modular-debiasing myth. While sparse MoEs offer elegant computational partitioning, our findings show that this modularity \textbf{does not translate into localized fairness properties}. Because bias-carrying experts are highly coupled with general linguistic capabilities (early-layer synergy $> 70\%$, and selectivity collapses to $\approx 1.08$ at scale), \textbf{surgical expert editing is an ineffective alignment strategy}. The experts whose ablation removes bias are the same high-traffic, general-purpose backbone units required to maintain core model performance; removing them selectively is fundamentally impossible without catastrophically breaking the model.
...
\begin{itemize}
    \item \textbf{Metric-Check Baseline Nature:} Our dense model evaluation in Experiment 2 serves strictly as a metric sanity check. We successfully resolved the player granularity and attribution method confounds in Experiment 8, demonstrating that the localizability gap stands even when using identical layer-level causal LOO mechanisms on both sides.
...
We present a post-hoc game-theoretic bias-attribution study on sparse Mixture-of-Experts architectures. Across a diverse model ladder spanning massive scales and routing sparsities, we show that social bias is an interactive, demographic-specific, and capability-coupled property of MoEs. We reject the simple surgical-separability thesis, proving causally that social stereotype bias rides on high-traffic, general-capability experts. Consequently, targeted expert ablation does not surgically remove bias, but instead catastrophically degrades general capability, yielding a selectivity metric of $\approx 1.08$ at scale.
...
\bibitem{winogender} Rudinger, R., Narayanan, C., Jafmin, B., \& Van Durme, B. (2018). Gender Bias in Coreference Resolution. \textit{Proceedings of the 2018 Conference of the North American Chapter of the Association for Computational Linguistics: Human Language Technologies (NAACL 2018)}, 8--14.
